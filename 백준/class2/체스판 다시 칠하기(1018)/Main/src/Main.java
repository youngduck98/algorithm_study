...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int c=1;c<=map[0].length-8;c++){
            int now1 = sum1[7][c+7] - sum1[7][c-1];
            int now2 = sum2[7][c+7] - sum2[7][c-1];
            if(min1 > now1)
                min1 = now1;
            if(min2 > now2)
                min2 = now2;
        ...
        for(int r=1;r<=map.length-8;r++){
            int now1 = sum1[r+7][7] - sum1[r-1][7];
            int now2 = sum2[r+7][7] - sum2[r-1][7];
            if(min1 > now1)
                min1 = now1;
            if(min2 > now2)
                min2 = now2;
        ...
        for(int r=1;r<=map.length-8;r++){
            for(int c=1;c<=map[0].length-8;c++){
                int now1 = sum1[r+7][c+7] -
                        (sum1[r+7][c-1] + sum1[r-1][c+7] - sum1[r-1][c-1]);
                int now2 = sum2[r+7][c+7] -
                        (sum2[r+7][c-1] + sum2[r-1][c+7] - sum2[r-1][c-1]);
                if(min1 > now1)
                    min1 = now1;
                if(min2 > now2)
                    min2 = now2;
